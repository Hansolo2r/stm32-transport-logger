...
- Electrical-connectivity correction: all single-row module headers now run horizontally across different numbered columns. The STM32 spans the center trench with its two 20-pin rows in B/I at columns 18-37; DS1302, W25Q64, and JDY-16 use separate A-row columns, while ADXL345, OLED, and DS18B20 use separate J-row columns. ADXL345 was moved to columns 40-47 and must be jumper-wired to PB10/PB11, rather than being directly inserted beneath unrelated STM32 pins.
- Final diagram validation: counted the rendered header contacts against the physical modules (STM32 20+20, DS1302 5, W25Q64 6, JDY-16 4, ADXL345 8, OLED 4, and DS18B20 3) and checked that no two modules share a numbered column on the same A-E or F-J half.

## 2026-08-08 - Mobile DS1302 time synchronization

- Observed issue: DS1302 returned an invalid time after final rewiring, but the complete `time set YYYY-MM-DD HH:MM:SS` command exceeded the dashboard's former 20-byte BLE write limit and USART1 was not connected.
- Dashboard change: added a `同步手机时间` control that formats the iPhone's local time, sends the existing firmware command, reads the clock back, and refreshes device status. No STM32 firmware or CubeMX change is required.
- BLE transport change: commands are encoded once with a single trailing newline and then written sequentially in chunks of at most 20 bytes, with a 30 ms gap between chunks. Existing short commands remain one write.
...
- Hardware validation: the user flashed the new image and confirmed all four requested behaviors on the assembled breadboard device: a short tap from rest produced shock without motion; pickup and continued movement for over one second produced motion without a duplicate startup shock; returning to rest ended motion; and a separate short impact while already moving produced an additional shock. Final threshold calibration after enclosure assembly remains separate from this functional validation.
